...
import { tokenOwner, tokenBalance } from "./rpc.js";
...
const PAIR_V5 = "0x8660a7f019c7943b0b0a91b8e39aff3b6db6ae62";
const PAIR_V5_IMPL = "0x1559f2beda73c57ab7964ed1a29d8c4d8370b12c";
const PAIR_AGG = "0x9d7741776098afa315e4d576ede4f2c67a21d8ce";
const PAIR_HOOK = "0x16d1560630ce74af4478d9b8ad46548a092a2000";
const PAIR_LOCKER = "0xefcf476e8870fb3eb8680f039414fdcce6c2a117";
const PAIR_ADAPTER = "0xa57472d29caf87011241249e830c924bf39bf333";
const PAIR_PAD_FACTORY = "0x9d33ba78389c8772bc114cba47dc1985e933e76f";
const PAIR_PAD_LOCKER = "0x8a6d37b2e6a2ac7970ef69d2932757f04be0a231";
const PAIR_PAD_DEPLOYER = "0x916c53ae4738196394c2661c5b0c71b84f40e36c";
const PAIR_PAD_MINTER = "0xdab26bb66f29863f2d68ced54f65cc614c4e65dc";
...
const PAIR_SET = new Set([PAIR_V5, PAIR_V5_IMPL, PAIR_AGG, PAIR_HOOK, PAIR_LOCKER, PAIR_ADAPTER, PAIR_PAD_FACTORY, PAIR_PAD_LOCKER, PAIR_PAD_DEPLOYER, PAIR_PAD_MINTER]);

const KNOWN = {
  "0x91a2dae9699f0b82540b5886b0d8759c22820ba3": "bankr",
  "0x786d42bc32fcea61e75c87128ed4ac144133bba3": "bankr",
  ...
  "0xab093def657f15df31b33922a95e047add645b29": "pons-v2",
...
function padFromAddr(addr) {
  const c = norm(addr);
  if (!c) return null;
  if (c === LONG_FACTORY || c === DOPPLER_FACTORY || c === DOPPLER_ERC20) return "long";
  if (c === AIRLOCK) return "airlock";
  if (PONS_V1_SET.has(c)) return "pons-v1";
  if (c === PONS_V3) return "pons-v3";
  if (PONS_V2_SET.has(c)) return "pons-v2";
  if (PAIR_SET.has(c)) return "pair";
...
function padFromImpl(impls) {
  for (const impl of impls || []) {
    ...
    if (mapped && mapped !== "airlock") return mapped;
    if (name.includes("doppler")) return "long";
    if (name.includes("bankr")) return "bankr";
    if (name.includes("ponsv3") || name.includes("pons v3")) return "pons-v3";
    if (name.includes("ponsv2") || name.includes("pons v2") || name.includes("v2launch")) return "pons-v2";
    if (name.includes("pons")) return "pons-v1";
    if (name.includes("pairlaunch") || name.includes("pairpad") || name.includes("pairv4") || name.includes("pairv5")) return "pair";
...
async function padFromLocker(address) {
  const token = norm(address);
  if (!token) return null;
  try {
    if (await tokenBalance(token, PONS_V2_LOCKER) > 0) return "pons-v2";
    if (await tokenBalance(token, PAIR_LOCKER) > 0) return "pair";
    if (await tokenBalance(token, PAIR_PAD_LOCKER) > 0) return "pair";
    if (await tokenBalance(token, PONS_V1_LOCKER) > 0) return "pons-v1";
    if (await tokenBalance(token, PONS_V1_LOCKER_OLD) > 0) return "pons-v1";
  } catch (e) {}
  return null;
}
...
export async function fillPads(coins, limit) {
  ...
  const need = coins.filter((c) => c && c.address).slice(0, limit || 80);
  await Promise.all(need.map(async (c) => {
    ...
    let onchain = await padFromLocker(addr);
    const owner = onchain ? null : await tokenOwner(addr);
    let creator = owner || null;
    if (!onchain) onchain = padFromAddr(owner);
    if (!onchain) {
      const extra = await inspect(addr);
      creator = extra.creator || creator;
      onchain = padFromAddr(extra.creator) || extra.pad;
    }
    if (!onchain && addr.endsWith("01")) onchain = "o1";
    c.padCreator = creator || null;
    c.padOwner = owner || null;
    c.padOnchain = Boolean(onchain && onchain !== "airlock");
    if (onchain && onchain !== "airlock") c.launchpad = onchain;
