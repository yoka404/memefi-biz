...
export async function fillPads(coins, limit) {
  ...
  const mapped = await creatorsBatch(need.map((c) => norm(c.address)));
  await Promise.all(need.map(async (c) => {
    const extra = await inspect(c.address);
    const creator = mapped[norm(c.address)] || extra.creator;
    const onchain = padFromCreator(creator) || extra.pad;
