...
function putCoin(bag, row) {
  ...
  const locked = Number(row.stockLockedUsd) || 0;
  const prevLocked = prev ? Number(prev.stockLockedUsd) || 0 : 0;
  const liq = Number(row.liquidityUsd) || 0;
  const prevLiq = prev ? Number(prev.liquidityUsd) || 0 : 0;
  if (prev && prevLocked >= locked && prevLiq >= liq) {
...
function ingestDexPair(bag, p, stocks) {
  ...
  if (quoteIsStock && qAmt > 0) {
    ...
    else if (px > 0 && qAmt > 0 && bAmt > 0) lockedUsd = qAmt * (px / (bAmt ? px : 1));
...
function wrapFromDex(pairs, stocks) {
  const out = {};
  const liq = {};
  for (const p of pairs || []) {
    if (!p || String(p.chainId || "").toLowerCase() !== "robinhood") continue;
    const base = norm(p.baseToken && p.baseToken.address);
    const quote = norm(p.quoteToken && p.quoteToken.address);
    const px = Number(p.priceUsd);
    const native = Number(p.priceNative);
    const usd = Number(p.liquidity && p.liquidity.usd) || 0;
    if (!(px > 0)) continue;
    let sym = null;
    let wrap = null;
    if (stocks.byAddr[base] && (quote === USDG || quote === WETH)) {
      sym = stocks.byAddr[base].symbol;
      wrap = px;
    } else if (stocks.byAddr[quote] && !stocks.byAddr[base] && native > 0) {
      sym = stocks.byAddr[quote].symbol;
      wrap = px / native;
    }
    if (!sym || !(wrap > 0)) continue;
    if (wrap > 0.4 && wrap < 1.6 && !/SGOV|USD|USDC/.test(sym)) continue;
    if (!out[sym] || usd > (liq[sym] || 0)) {
      out[sym] = wrap;
      liq[sym] = usd;
    }
  }
...
async function dexPairs(addrs) {
  const out = [];
  const chunks = [];
  for (let i = 0; i < addrs.length; i += 4) chunks.push(addrs.slice(i, i + 4));
  for (let i = 0; i < chunks.length; i += 6) {
    const slice = chunks.slice(i, i + 6);
    const bags = await Promise.all(slice.map(async (chunk) => {
      try {
    ...
    for (const bag of bags) for (const p of bag) out.push(p);
  }
  return out;
}
...
export async function buildUniverse() {
  ...
  for (const kind of ["trending_pools", "new_pools", "pools"]) {
    for (let page = 1; page <= 6; page++) geckoPaths.push("/" + kind + "?page=" + page + "&include=base_token,quote_token");
  }
  ...
  const onchain = wrapFromDex(dexPairsFound, stocks);
  for (const c of coins) {
    if (c.pair && c.onchain != null && onchain[c.pair] == null) {
      const w = Number(c.onchain);
      if (w > 0 && !(w > 0.4 && w < 1.6 && !/SGOV|USD/.test(c.pair))) onchain[c.pair] = w;
    }
  }
  return {
