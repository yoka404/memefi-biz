...
async function dexForWrappers(stocks) {
  const addrs = stocks.assets.map((a) => a.address).filter(Boolean);
  const bagPairs = [];
  for (let i = 0; i < addrs.length; i += 25) {
    ...
    try {
      ...
      for (const p of pairs) bagPairs.push(p);
    } catch (e) {}
  }
  return bagPairs;
}
...
export async function buildUniverse() {
  ...
  const bag = {};
  const dexPairs = await dexForWrappers(stocks);
  for (const p of dexPairs) ingestDexPair(bag, p, stocks);
  const geckoPaths = [
