...
function ingest(bag, page, stocks) {
  ...
  for (const p of page.pools) {
    ...
    if (prev && Number(prev.liquidityUsd || 0) >= (Number.isFinite(liq) ? liq : 0)) {
      if (!prev.geckoImage && memeTok.image) prev.geckoImage = memeTok.image;
      continue;
    }
    bag[memeAddr] = {
      ...
      geckoImage: memeTok.image || (prev && prev.geckoImage) || null,
      listed: true
    };
  }
}
export async function buildUniverse() {
  ...
  for (const kind of ["trending_pools", "new_pools", "pools"]) {
    for (let page = 1; page <= 5; page++) paths.push("/" + kind + "?page=" + page + "&include=base_token,quote_token");
  }
  const dexes = ["pons-v2", "pons-v2-dex", "pons-dot-family", "bankr-robinhood", "uniswap-v4", "uniswap-v4-robinhood", "long-xyz", "doppler", "feel-cash", "flap"];
  for (const d of dexes) {
    paths.push("/dexes/" + d + "/pools?page=1&include=base_token,quote_token");
    paths.push("/dexes/" + d + "/pools?page=2&include=base_token,quote_token");
  }
  ...
  const pairs = new Set(coins.map((c) => c.pair).filter(Boolean));
  return {
